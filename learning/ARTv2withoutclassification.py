...
"""
import datetime #print("Current Time:", datetime.datetime.now())
import os
import numpy as np
import time
import requests
...
class FuzzyART:
    ...
    def update_fuzzy_art_vigilance(self, new_vigilance):
        """
        Update the vigilance parameter of the FuzzyART instance.
        """
        self.fuzzy_art.vigilance = new_vigilance
        print(f"Vigilance updated to: {self.fuzzy_art.vigilance}")
...
class DataProcessor:
    ...
    def _extract_event_data(self, perception_data):
        ...
        # print(image, content_length_image, text, content_length_text) 
...
class RLAgent:
    ...
    def update_rl_instance(self, category, config_index, reward, response_time, feature):
        ...
        # update min and max norm when multiple of 10 input
        # if len(response_times) >= 5:
        #     # get the most recent 10 response time
        #     recent_times = response_times[-5:]
            # count_above_100 = sum(1 for time in recent_times if time > 100)
            # if count_above_100 > 5:
        ...
        if response_time > 100:
            ...
            # Calculate the 5th and 95th percentiles for the window
            # min_norm = np.percentile(recent_times, 5)
            # max_norm = np.percentile(recent_times, 95)
...
class MLSystem:
    ...
    def run(self):
        ...
        while iteration < self.config.MAX_ITERATIONS:
            ...
            reward, response_time = self.reward_calculator.calculate_reward(perception_data.get('metrics', []))
